...
Currency exchange rates against the Brazilian real — web, Android, Linux and
Ubuntu Touch. Website: <https://hhldiniz.github.io/cotacao_direta/> · web
app: <https://hhldiniz.github.io/cotacao_direta/app/>.
...
O `id` do manifest está fixado em `/cotacao_direta/`: é ele que dá identidade
estável ao app instalado. O app hoje é publicado em `/cotacao_direta/app/` (ver
[Public website and SEO](#public-website-and-seo)), mas o `id` continua o do
caminho antigo de propósito — trocá-lo faria o navegador tratar o app como
outro, e quem já o instalou ficaria com duas cópias. O `id` não precisa estar
dentro do `scope`, só na mesma origem.
...
Flutter web draws the whole UI on a canvas, so search engines see almost
nothing in the app itself. The GitHub Pages deploy therefore publishes two
things side by side:

| Path | What it is |
| --- | --- |
| `/cotacao_direta/` | Static landing page in pt-BR (`/en/` and `/es/` for English and Spanish, linked with `hreflang`) |
| `/cotacao_direta/cotacao/` | Today's rates, plus one page per currency (`/cotacao/dolar/`, `/cotacao/euro/`, `/cotacao/bitcoin/`…) with the current rate, 30-day chart and history, conversion tables and explanatory text |
| `/cotacao_direta/app/` | The Flutter app (PWA), marked `noindex` |
| `sitemap.xml`, `robots.txt`, `404.html` | Generated with the pages |

The static pages are plain HTML generated by `tool/build_site.dart` (code in
`tool/site/`, tests in `test/tool/`). They fetch rates from AwesomeAPI at build
time and carry the usual SEO metadata: title and description, `canonical`,
Open Graph/Twitter card (`site/og-image.png`), JSON-LD (`WebApplication` on the
landing pages, `BreadcrumbList` on the rest) and `hreflang`. Styles live in
...
Preview locally (needs network access to AwesomeAPI):

```sh
flutter build web --release --no-web-resources-cdn --base-href /app/
dart run tool/build_site.dart --out build/site --site-url http://localhost:8080/
cp -r build/web build/site/app
dart run dhttpd --path build/site --port 8080
```

### Moving the app to `/app/`

Before this change the app lived at the site root. Existing installs and the
service worker registered there are migrated automatically: the static pages
unregister any service worker whose scope is the site root, and the root page
sends visitors that opened it from the installed app icon (standalone display
mode) to `app/`. There the browser reads the manifest, which keeps the same
`id`, and updates the installed app's `start_url`. The service worker cache
name was bumped to `cotacao-direta-v2` to drop the old cached shell.

### Manual steps (outside the repository)

1. **Google Search Console**: add a *URL prefix* property for
   `https://hhldiniz.github.io/cotacao_direta/`, choose the *HTML tag*
   method, and put the token (the `content` value only) in the repository
   variable `GOOGLE_SITE_VERIFICATION` (Settings → Secrets and variables →
   Actions → Variables). Redeploy, verify, then submit `sitemap.xml`.
2. **Bing Webmaster Tools** (also feeds DuckDuckGo and others): same idea with
   the `BING_SITE_VERIFICATION` variable, or import the site from Search
   Console.
3. **`robots.txt`** is only read at the domain root, so on
   `hhldiniz.github.io/cotacao_direta/` it is ignored; submitting the sitemap
   by hand covers it. It starts working as-is with a custom domain.
4. **Custom domain** (optional, recommended): configure it under Settings →
   Pages. The workflow reads the site URL and base path from
   `actions/configure-pages`, so canonical URLs, the sitemap and the app's
   `--base-href` follow automatically.
5. **Backlinks**: set the repository's *Website* field to the site URL and
   add topics (`cotacao`, `cambio`, `dolar`, `currency-converter`, `pwa`,
   `flutter`); link the site from store listings (the Flathub metainfo
   homepage already points to it).
