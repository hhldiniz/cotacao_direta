...
class HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    ...
    final HomeBloc _bloc = HomeBlocProvider.of(context);

    WidgetsBinding.instance
        .addPostFrameCallback((_) => _bloc.getSelectedOverrideCurrency());

    final pageHeader = StreamBuilder(builder: (BuildContext context, snapshot) {
      return Text(
        sprintf(_localization.homePageHeadsUpText, [snapshot.data]),
        style: TextStyle(fontSize: 28),
      );
    }, initialData: "", stream: _bloc.headsUpTextStream,);
